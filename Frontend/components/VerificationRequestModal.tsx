...
const VerificationRequestModal: React.FC<VerificationRequestModalProps> = ({ posting, onApprove, onReject }) => {
  ...
  return (
    <div className="fixed inset-0 z-[300] bg-white/80 dark:bg-slate-900/80 backdrop-blur-md flex items-center justify-center p-4 animate-fade-in-up">
      <div className="bg-white rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl relative">
        <div className="bg-white dark:bg-slate-900 p-6 text-slate-900 dark:text-white text-center">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm font-bold mt-1">{subtitle}</p>
        </div>
        ...
        <div className="p-6">
            <div className="rounded-2xl overflow-hidden bg-slate-100 border border-slate-200 mb-6 relative">
                ...
                />
                <div className="absolute bottom-0 left-0 right-0 bg-black/60 backdrop-blur-sm p-2 text-center text-slate-900 dark:text-white text-xs font-bold uppercase tracking-widest">
                    {proofLabel}
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
                ...
                <button 
                    onClick={onApprove}
                    className="py-4 bg-emerald-600 text-slate-900 dark:text-white hover:bg-emerald-700 rounded-2xl font-black uppercase text-xs tracking-wider transition-colors shadow-lg shadow-emerald-200 flex items-center justify-center gap-2"
                >
