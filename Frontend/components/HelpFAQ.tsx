        ...
        <div className="bg-slate-50 p-6 md:p-8 md:w-64 border-b md:border-b-0 md:border-r border-slate-100 flex-shrink-0">
            ...
            <div className="flex md:flex-col gap-2 overflow-x-auto md:overflow-visible pb-2 md:pb-0 scrollbar-hide">
                {faqs.map((section) => (
                    <button
                        ...
                        className={`px-4 py-3 rounded-xl text-xs font-bold uppercase tracking-wider text-left transition-all whitespace-nowrap ${activeCategory === section.category ? 'bg-white dark:bg-slate-900 text-slate-900 dark:text-white shadow-lg' : 'text-slate-500 hover:bg-white hover:shadow-sm'}`}
                    >
                        {section.category}
                    </button>
                ))}
            ...
            <div className="mt-8 hidden md:block">
                <div className="bg-emerald-50 rounded-2xl p-5 border border-emerald-100">
                    ...
                    <button onClick={onContact} className="w-full py-2 bg-emerald-600 text-slate-900 dark:text-white rounded-lg text-xs font-black uppercase hover:bg-emerald-700 transition-colors">Contact Support</button>
                </div>
            </div>
        </div>
                ...
                {currentFaqs.map((item, idx) => (
                    <div key={idx} className="border border-slate-100 rounded-2xl overflow-hidden transition-all duration-300 hover:border-slate-200">
                        ...
                        >
                            ...
                            <span className={`transform transition-transform duration-300 text-slate-500 dark:text-slate-400 ${openQuestionIndex === idx ? 'rotate-180' : ''}`}>
